...
def user_registration(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('catalog_book_page')
        messages.error(request, 'Данные заполнены не корректно')
    else:
...
def user_login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('catalog_book_page')
        messages.error(request, 'Данные не корректно заполнены')
    else:
        form = LoginForm()
...
def anonim(request):
    ...
    print('Может добавить издательства?', request.user.has_perm('library.add_publishing_house'))
...
def change_only_telephone_publishing_house(request):
